...
- 数据导入需求: 管理员提供两种资料：1.按年级分开的单词表（6-9年级）；2.网上整理的初中词转表（贯穿初中，包含所有中考词转知识）。程序需基于词转表生成词族，并关联单词表中的数据。
- **向下兼容需求**: 8年级学生应能看到6年级和7年级的单词和词转内容，高年级学生可以学习当前年级内容，同时复习所有之前年级的内容。
...
- **数据策略**：单词表严格按年级（6-9年级）；词转、搭配、语法表贯穿初中，不按年级隔离。
- **词族生成**：基于词转表（主要数据源）生成词族，关联单词表数据，根据用户年级推荐。
- **向下兼容**：6年级→6年级；7年级→6+7年级；8年级→6+7+8年级；9年级→6+7+8+9年级。
...
### 文件创建（向下兼容系统）
- `src/storage/database/learningContentAdapter.ts` (学习内容适配器，核心向下兼容逻辑)
- `src/app/api/learning/content/route.ts` (获取学习内容 API)
- `src/app/api/learning/stats/route.ts` (获取学习统计 API)
- `src/app/api/learning/plan/route.ts` (生成学习计划 API)
- `src/app/api/learning/test-compatibility/route.ts` (测试向下兼容 API)
...
- `src/app/api/admin/words/import/route.ts` (导入单词)
...
### 文件创建（测试）
- `src/storage/database/test-downward-compatibility.ts` (向下兼容逻辑测试脚本)
...
### 数据库结构设计
- **word_families 表**：词族核心表，包含 base_word、grade（主要年级）、stats
- **user_word_family_progress 表**：追踪词族学习进度（支持艾宾浩斯记忆曲线）
- **words 表**：添加 grade 字段，严格按年级隔离
- **word_transformations 表**：不添加 grade 字段（贯穿初中）
- **collocations 表**：不添加 grade 字段（贯穿初中）
...
- **学习内容适配器（Learning Content Adapter）**：核心模块
  - `getAccessibleGrades(userGrade)`：获取用户可访问的年级列表
  - `adaptWordFamiliesForUser(userGrade, limit)`：为用户适配词族（向下兼容）
  - `getUserLearningStats(userId, userGrade)`：获取学习统计
  - `generateLearningPlan(userGrade)`：生成学习计划
- **年级映射**：
  ```typescript
  GRADE_COMPATIBILITY = {
    '6年级': ['6年级'],
...
- **问题**: Netlify 数据库迁移执行失败。
  - **解决方案**: 拆分迁移脚本。Step1 创建4个新表，Step2 创建4个缺失的表，Step3 添加年级字段。
...
2. 测试 API 接口：
   - `/api/learning/content` - 获取学习内容
   - `/api/learning/stats` - 获取学习统计
   - `/api/learning/plan` - 生成学习计划
   - `/api/learning/test-compatibility` - 测试向下兼容（管理员）
